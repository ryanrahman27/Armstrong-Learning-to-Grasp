...
class EnvironmentVisualizer(Node):
    ...
    def publish_markers(self):
        ...
        camera_marker.id = 3
        camera_marker.type = Marker.LINE_LIST
        camera_marker.action = Marker.ADD
        camera_marker.pose.orientation.w = 1.0
        camera_marker.scale.x = 0.01  # Line width
        camera_marker.color.r = 0.0
        camera_marker.color.g = 1.0
        camera_marker.color.b = 1.0
        camera_marker.color.a = 1.0
        
        # Simple camera representation - a small box at camera location
        cam_x, cam_y, cam_z = 0.5, 0.0, 1.0
        camera_marker.points = [
            Point(x=cam_x, y=cam_y, z=cam_z),
            Point(x=cam_x + 0.1, y=cam_y, z=cam_z - 0.1),
        ]
